...
test("applies correct CSS classes based on loading state", () => {
  ...
  expect(submitButton.className).toContain("hover:bg-violet-500/10");
...
test("applies pulse animation to send icon when loading", () => {
  ...
  expect(sendIcon?.getAttribute("class")).toContain("text-gray-600");
});
...
test("textarea has correct styling classes", () => {
  ...
  expect(textarea.className).toContain("focus:ring-violet-500/20");
});
