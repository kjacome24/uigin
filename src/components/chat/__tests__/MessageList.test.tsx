...
test("MessageList shows empty state when no messages", () => {
  ...
  expect(
    screen.getByText("Describe a React component and I will generate it with live preview")
  ).toBeDefined();
});
...
test("MessageList applies correct styling for user vs assistant messages", () => {
  ...
  expect(userMessage?.className).toContain("from-violet-500");
  expect(userMessage?.className).toContain("text-white");

  // Assistant messages should have dark glass background
  expect(assistantMessage?.className).toContain("bg-white/[0.06]");
  expect(assistantMessage?.className).toContain("text-gray-100");
});
...
test("MessageList shows loading for assistant message with empty parts", () => {
  ...
  const loadingText = container.querySelectorAll(".text-gray-400");
