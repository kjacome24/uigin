...
function FileTreeNode({ node, level }: FileTreeNodeProps) {
  ...
  return (
    <div>
      <div
        className={cn(
          "flex items-center gap-2 px-2 py-1.5 hover:bg-gray-100 cursor-pointer text-sm transition-colors duration-150 rounded-md mx-1",
          selectedFile === node.path && "bg-violet-50 text-violet-600"
        )}
      ...
      >
        {node.type === "directory" ? (
          <>
            {isExpanded ? (
              <ChevronDown className="h-3.5 w-3.5 shrink-0 text-gray-400 transition-transform duration-150" />
            ) : (
              <ChevronRight className="h-3.5 w-3.5 shrink-0 text-gray-400 transition-transform duration-150" />
            )}
            {isExpanded ? (
          ...
          <>
            <div className="w-3.5" />
            <FileCode className="h-4 w-4 shrink-0 text-indigo-400/60" />
          </>
        )}
        <span className="truncate text-gray-600">{node.name}</span>
      </div>
  ...
  if (!rootNode || !rootNode.children || rootNode.children.size === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
        <Folder className="h-12 w-12 text-gray-300/60 mb-3" />
